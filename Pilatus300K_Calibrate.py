...
def read_TIFF(file):
    ...
    try:
        ...
        for i in xrange(0, 5):
            arr[:,i] = -2.0
        for i in xrange(471, 504):
            arr[:,i] = -2.0
        for i in xrange(965, 998):
            arr[:,i] = -2.0
...
# Read CSV file, get step size and npber of points in scan
...
# Read images, take line cut, fit peak for calibration scan
pks = []
plt.figure()
for i in xrange(0, np_points):
    print i
...
lin_fit, pcov = curve_fit(simple_line, pks, x*calib_tth_steps + 10.00)
